...
### UI-ViewModel Pattern (UDF)
We use a **Unidirectional Data Flow (UDF)** pattern for UI components:
- **UiState**: A single StateFlow exposed by the ViewModel representing the current state of the UI.
- **UiEvent**: A sealed interface representing user interactions or events sent to the ViewModel.
- **ViewModel**: Handles events via an `onEvent` method and updates the `UiState`.
